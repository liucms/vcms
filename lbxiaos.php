...
if(!empty($_GET['id'])&&!empty($_GET['n'])) {
    $sum = 3;
    $data = array();
    $list = array(19,18,21,22,23,24); // 采集ID
    ...
    echo '第 '.intval($_GET['id']).' 页 栏目 '.intval($_GET['n']).PHP_EOL.'<script>window.location.href="/lbxiaos.php?id='.$pid[0].'&n='.$pid[1].'";</script>';
}

function getNews($cid = 1, $p = 1) {
    $jsonVideo = getJson('https://lbapi9.com/api.php/provide/art/?ac=detail&t='.intval($cid).'&pg='.intval($p).'&h=&ids=&wd=',getRandIP());
